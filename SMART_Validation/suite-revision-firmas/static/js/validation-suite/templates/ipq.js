/* ====================================================================
   VALIDATION SUITE — RENDERER IPQ
   Informe de Calificación de Performance (Performance Qualification
   Report). Documento que reporta los resultados de ejecución de los
   TCs definidos en el PPQ. Misma estructura que el PPQ + secciones
   extras de hallazgos y conclusión.

   Diferencias clave vs PPQ:
   ...
   - Sección nueva: Resumen de Ejecución PQ (estadística PASS/FAIL/OBS
     con conteo separado de TCs negativos).
   - Sección nueva: Hallazgos consolidados.
   - Sección nueva: Conclusión y decisión formal (apto/no apto para uso
     productivo o liberación).
   ...
   - resumen-ejecucion-pq : tabla con totales (PASS/FAIL/OBS/N/A) +
...
(function (global) {
    ...
    if (!VS) {
        console.error('[ipq.js] ValidationSuite no esta cargado');
        return;
    }

    // ====================================================================
    // TIPO NUEVO: RESUMEN-EJECUCION-PQ
    // (variante del resumen-ejecucion-oq con label adaptado a PQ)
    // ====================================================================
    function renderResumenEjecucionPq(sec, tb, documentData) {
                ...
                table: {
                    ...
                    body: [[{
                        text: [
                            ...
                            { text: 'rechazaron correctamente el escenario adverso ', fontSize: 9, italics: true, color: C.text },
                            totalesNeg.fail > 0
                                ? { text: `(${totalesNeg.fail} FAIL — el sistema NO rechazó cuando debía).`, fontSize: 9, italics: true, color: '#A52A2A', bold: true }
                                : { text: '— cobertura de escenarios adversos demostrada.', fontSize: 9, italics: true, color: C.textSoft }
                        ],
                        fillColor: negBg,
                        margin: [12, 8, 12, 8]
                    }]]
        ...
        const labelDecision = aprobado
            ? (conObs ? 'PQ APROBADA CON OBSERVACIONES' : 'PQ APROBADA')
            : (totales.total === 0 ? 'EJECUCIÓN PENDIENTE' : 'PQ NO APROBADA');
            ...
            table: {
                ...
                body: [[{
                    stack: [
                        ...
                        {
                            text: aprobado
                                ? `${totales.pass}/${totales.total} TCs PASS${conObs ? ` (${totales.obs} con observaciones menores)` : ''}. ${totalesNeg.total > 0 ? `Los ${totalesNeg.total} TCs negativos demostraron cobertura de escenarios adversos. ` : ''}El sistema cumple los criterios de performance y está apto para uso productivo.`
                                : (totales.total === 0
                                    ? 'Aún no se han ejecutado test cases.'
                                    : `${totales.fail} TC(s) con FAIL requieren NCs documentadas y CAPA antes de liberar para uso productivo.`),
    ...
    function renderHallazgosConsolidados(sec, tb, documentData) {
        ...
        if (todos.length === 0) {
            out.push({
                table: {
                    widths: ['*'],
                    body: [[{
                        text: 'Sin hallazgos ni desvíos en la ejecución de la PQ.',
        ...
        out.push({
            text: cerradas === todos.length
                ? `Total: ${todos.length} hallazgo(s). Todas las NC fueron cerradas con su CAPA verificada antes del cierre del IPQ.`
    ...
    // RENDERER PRINCIPAL
    // ====================================================================
    VS.registerRenderer('IPQ', function renderIPQ(data) {
        ...
        // El modo es siempre 'informe' para IPQ
        const MODE = 'informe';
        ...
        secciones.forEach((sec, idx) => {
            ...
            switch (sec.tipo) {
                ...
                case 'resumen-ejecucion-pq': contentBlock = renderResumenEjecucionPq(sec, tb, data); break;
